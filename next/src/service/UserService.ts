...
// weights = "[[4, 2, 3],[2, 0, 0],[3, 1, 2],[2, 0, 0],[2, 0, 0],[2, 0, 0]]"
// days = "[1, 3, 2, 6, 18, 54]"
// nextDate = "2024-05-24"
  async linkUserWord(token: string, wordId: number, weights: string, days: string, nextDate: string): Promise<void> {
    try {
      await this.axiosInstance.post("/user/user-word",
        {wordId, weights, days, nextDate},
        {
          headers: {Authorization: token, 'Content-Type': 'application/json'}
        });
    } catch (error) {
  ...
  async updateLinkedUserWord(token: string, wordId: number, weights: string, days: string, nextDate: string): Promise<void> {
    try {
      await this.axiosInstance.put("/user/user-word",
        {wordId, weights, days, nextDate},
        {
          headers: {Authorization: token, 'Content-Type': 'application/json'}
        });
